...
//		sleepTime(5);
		...
		System.out.println(ownersSubNavigation);
		...
		for(int j=0;j<=subNavsize;j++) {
			for(int i=1;i<=subNavsize;i++) {
				...
				System.out.println(eachSubNavtext);
		...
		System.out.println("Owner TC completed");
		return this;	
	}
...
//		closeWelcomePopup();
		...
		for(int j=0;j<=headerNav.size();j++) {
			...
			for(int i=1;i<=headerNav.size();i++) {
				...
				System.out.println(headerClicktext);
	...
	public MiscPages homeHeaderNav(String Testcasenumber) {
		...
		sleepTime(30);
		clickByXpathjs(objValue.getProperty("xpath.dealerNav"), "Dealer section navigation");
		...
		if(dealersPageVerify.contains("Local Dealer")) {
		...
		System.out.println("Dealer page TC completed");
		return this;
	}
...
//		closeNotificationPopup();
		...
		//	closeWelcomePopup();
			...
			if(i==2) {
				...
				System.out.println(tradeValueTitle);
		...
		System.out.println("Trade in TC completed");
		return this;
	}
...
//		closeNotificationPopup();
		...
		System.out.println("Subnav TC completed");
		return this;
...
//		sleepTime(10);
		...
		sleepTime(5);
		clickByXpathjs(objValue.getProperty("Xpath.News"), "News Link");
		closeWelcomePopup();
		sleepTime(5);
		...
		for(int i=1;i<=newsSubSec.size();i++) {
			...
			if(eachSubNavtext.equalsIgnoreCase("The latest Hyundai news")) {
				...
				for(int j=1;j<=socialLinks.size();j++) {
					...
					switchToLastWindow();
					sleepTime(10);
					String socialTitle = driver.getPageSource();
					System.out.println("Social Title"+socialTitle);
					driver.close();
					switchToParentWindow();
					sleepTime(5);

					if(socialTitle.contains("facebook")) {
		...
		System.out.println("Socil icon TC completed");
		return this;
	}
	...
	public MiscPages autoShowsVerify(String Testcasenumber) throws InterruptedException {
		...
		String[] subSection= {"Gallery","Social","Keep in touch."};
...
//		closeWelcomePopup();
		...
		System.out.println("featureSectionText :"+featureSectionText);
		...
		System.out.println("socialSectionText "+socialSectionText);

		WebElement scheduleSection = driver.findElement(By.xpath(objValue.getProperty("xpath.autoKeepInTouchSubSection")));
		String scheduleSectionText = scheduleSection.getText();
		System.out.println("scheduleSectionText :"+scheduleSectionText);

		for(int j=0;j<=subSection.length;j++) {
             System.out.println("length :"+subSection.length);
			if(subSection[j].equalsIgnoreCase(featureSectionText)) {
				System.out.println("Hyundai Autoshows :" +featureSectionText+ " sub section verified successfully");
...
/*
				...
				System.out.println("Hyundai Autoshows :"+socialSectionText+ " sub section verified successfully");
				...
				for(int k=1;k<=socialSectionVerify.size();k++) {
                   ...
                   System.out.println("eachSocialIcon :"+eachSocialIcon.getText());
					js.executeScript("arguments[0].click();", eachSocialIcon);
		...
		System.out.println("Auto shows TC completed");
		return this;
	}
...
//		closeNotificationPopup();
		...
		if(driver.findElement(By.xpath(objValue.getProperty("xpath.comparePageVerify"))).isDisplayed()) {
			...
			clickByXpath(objValue.getProperty("xpath.compareModel"), "Compare Model");
			...
			clickByXpath(objValue.getProperty("xpath.compareTrim"), "Compare Model");
			sleepTime(10);
			...
			sleepTime(10);
			clickByXpathjs(objValue.getProperty("xpath.CompaddVehicle"), "Add vehicle button");
			
			selectByXpathIndex(objValue.getProperty("xpath.CompselectYear"),1);
			sleepTime(5);
			selectByXpathIndex(objValue.getProperty("xpath.CompselectMake"),1);
			sleepTime(5);
			selectByXpathIndex(objValue.getProperty("xpath.CompselectModel"),1);
			sleepTime(10);
			selectByXpathIndex(objValue.getProperty("xpath.CompselectTrim"),1);
			sleepTime(10);
			clickByXpathjs(objValue.getProperty("xpath.comAddButton"), "Compare button is clicked from header");
			sleepTime(5);
	...
	public MiscPages comparePageSectionVerify(String Testcasenumber) {
		...
		if(driver.findElementByXPath(objValue.getProperty("xpath.features"))!=null) {
			...
			if(driver.findElementByXPath(objValue.getProperty("xpath.specifications"))!=null) {
				...
				if(driver.findElementByXPath(objValue.getProperty("xpath.meachanical"))!=null) {
				clickByXpath(objValue.getProperty("xpath.meachanical"), "meachanical");	
		...
		System.out.println("Image TC completed");
		return this;
...
//		closeNotificationPopup();
//		sleepTime(5);
		String[] vehFooterLinks = {"Choose Your Hyundai","Hyundai SUVs","Hyundai Sedans","Hyundai Compacts","Hyundai Alternative Fuel Vehicles","N","Hyundai Vehicle Brochures","Genesis USA"};
		String[] vehFooterNav= {"All Vehicles","SUVs","Sedans","Compacts","Alt-Fuel","Performance","Vehicle Brochures","Genesis"};
			...
			for(int k=0;k<=vehFooterNav.length;k++) {	
				for(int i=1;i<=vehFooterLinks.length;i++) {
					...
					if(driver.findElement(By.xpath(objValue.getProperty("xpath.footerVehicles"))).isDisplayed()) {
						...
						else if(vehFooterNav[k].equals("Performance")) {

							WebElement allVehHeader = driver.findElement(By.xpath(objValue.getProperty("xpath.footerPerformanceVerify")));
							String allVehVerify = allVehHeader.getText();

							if(vehFooterLinks[j].equalsIgnoreCase(allVehVerify)) {
								reportStep("Peformance Page verified successfully", "PASS", true);

							}
							else {

								reportStep("Error occured in the Peformance Page verification", "FAIL", true);
...
//		husaLivePopupClose();
		...
		if(driver.findElementByXPath(objValue.getProperty("xpath.WhyHeaderText"))!=null) {
			...
			System.out.println("Why page"+whyPage);
...
//		closeWelcomePopup();
		if(driver.findElementByXPath(objValue.getProperty("xpath.madeAmericaLink"))!=null) {
			clickByXpathjs(objValue.getProperty("xpath.madeAmericaLink"), "Made in America link");
			sleepTime(10);
			...
			if(madePage.contains("Building cars all")) {
...
//		closeWelcomePopup();
		...
		System.out.println("TC028 completed");
		return this;
...
//		closeWelcomePopup();
		...
		sleepTime(10);
		WebElement scrollupshopperLink = driver.findElementByXPath("(//a[@class='global-header-feature-cta'])[1]");
		js.executeScript("arguments[0].scrollIntoView(true);",scrollupshopperLink);
...
//		closeWelcomePopup();
		...
		System.out.println("TC029 completed");
...
//		closeWelcomePopup();
		...
		System.out.println("TC030 completed");
		return this;
...
//		closeWelcomePopup();
		...
		if(driver.findElementByXPath(objValue.getProperty("xpath.applyVehicleDetails"))!=null) {
			...
			clickByXpath(objValue.getProperty("xpath.msrpcloseButton"), "Close button");
...
//		closeWelcomePopup();
		...
		WebElement applypopup=driver.findElement(By.xpath("//h1[@class='BodyCopy Emphasis White']"));
		System.out.println("text :"+applypopup.getText());
		...
		System.out.println("Apply for credit TC completed");
		System.out.println("TC031 completed");
		return this;
...
//		closeWelcomePopup();
		...
		System.out.println("TC32 completed");
		return this;
...
//		closeWelcomePopup();
		...
		int j=5;
		WebElement vehilcleModelName = driver.findElementByXPath("(//div[@class='vc-name'])");
		String compactsVehicleName = vehilcleModelName.getText();
		System.out.println("compactsVehicleName :"+compactsVehicleName);
		...
		System.out.println("TC33 completed");
		return this;
...
//		closeWelcomePopup();
		...
		System.out.println("TC34 completed");
		return this;
...
//		closeWelcomePopup();
		...
		if(driver.findElementByXPath(objValue.getProperty("xpath.researchSection"))!=null) {
			...
			List <WebElement> menuList = driver.findElementsByXPath("((//nav[@class='global-header-dropdown-column'])[1]//a)");
				...
				for(int k=0;k<=linkNavHeader.length;k++) {           
					for(int j = 1;j<=menuList.size();j++) {
						...
						if(reseachLink[i].equals("Why Hyundai")) {
							...
							WebElement listMenu = driver.findElementByXPath("((//nav[@class='global-header-dropdown-column'])[1]//a)[" +(j+0)+ "]");
							
							String linksName = listMenu.getText();
							reportStep("Both links " +reseachLink[i]+ " & " +linksName+ " are displayed same", "PASS", true);
                            sleepTime(10);
							listMenu.click();
							System.out.println(linksName+": clicked");
...
//		closeWelcomePopup();
		...
		if(driver.findElementByXPath(objValue.getProperty("xpath.shopSection"))!=null) {
			...
			String reseachLink[] = {"Shop Hyundai","Find a Dealer","Build & Price","Search New Inventory","Schedule a Test Drive","Search Certified Pre-Owned"};
			String linkNavHeader [] = {"Shop Hyundai","Hyundai Dealerships near","Build Your Hyundai","Find Your Hyundai","Schedule a Hyundai Test Drive","Hyundai Certified Pre-Owned"};

			List <WebElement> menuList = driver.findElementsByXPath("((//nav[@class='global-header-dropdown-column'])[2]//a)");
			for(int i=0;i<=reseachLink.length;i++) {
				for(int k=0;k<=linkNavHeader.length;k++) {           
					for(int j = 1;j<=menuList.size();j++) {
						
						if(reseachLink[i].equals("Shop Hyundai")) {
							WebElement listMenu = driver.findElementByXPath("((//nav[@class='global-header-dropdown-column'])[2]//a)[" +(j+0)+ "]");
							String linksName = listMenu.getText();
							reportStep("Both links " +reseachLink[i]+ " & " +linksName+ " are displayed same", "PASS", true);
							...
							sleepTime(15);
							System.out.println(linksName+": clicked");
							String headerVerify = getTextByXpath(objValue.getProperty("xpath.shopHyundaiPageVerify"));
						...
						else if(reseachLink[i].equals("Find a Dealer")) {
							WebElement listMenu = driver.findElementByXPath("((//nav[@class='global-header-dropdown-column'])[2]//a)[" +(j+0)+ "]");
							String linksName = listMenu.getText();
							reportStep("Both links " +reseachLink[i]+ " & " +linksName+ " are displayed same", "PASS", true);
							sleepTime(15);
...
//		closeWelcomePopup();
		...
		if(driver.findElementByXPath(objValue.getProperty("xpath.buySection"))!=null) {
			...
			List <WebElement> menuList = driver.findElementsByXPath("((//nav[@class='global-header-dropdown-column'])[3]//a)");
			for(int i=0;i<=reseachLink.length;i++) {
				for(int k=0;k<=linkNavHeader.length;k++) {           
					for(int j = 1;j<=menuList.size();j++) {

						if(reseachLink[i].equals("Offers & Promotions")) {
							WebElement listMenu = driver.findElementByXPath("((//nav[@class='global-header-dropdown-column'])[3]//a)[" +(j+0)+ "]");
							String linksName = listMenu.getText();
							reportStep("Both links " +reseachLink[i]+ " & " +linksName+ " are displayed same", "PASS", true);
						...
						else if(reseachLink[i].equals("Car and Driver Trade-in")) {
							WebElement listMenu = driver.findElementByXPath("((//nav[@class='global-header-dropdown-column'])[3]//a)[" +(j+0)+ "]");
							String linksName = listMenu.getText();
							reportStep("Both links " +reseachLink[i]+ " & " +linksName+ " are displayed same", "PASS", true);
...
//		closeWelcomePopup();
		...
		if(driver.findElementByXPath(objValue.getProperty("xpath.ownerSection"))!=null) {
			...
			String reseachLink[] = {"Login to MyHyundai","Make a Payment","Schedule Service","Owner's Manuals","24/7 Roadside Assistance","Bluelink® Multimedia/Map Updates"};
			String linkNavHeader [] = {"MyHyundai","","Hyundai Dealerships near","Hyundai Resources","Hyundai Assurance 24/7 Roadside Assistance","Navigation Updates"};

			List <WebElement> menuList = driver.findElementsByXPath("(//nav[@class='global-header-dropdown-column global-header-dropdown-more-container']//a)");
			for(int i=0;i<=reseachLink.length;i++) {
				for(int k=0;k<=linkNavHeader.length;k++) {           
					for(int j = 1;j<=menuList.size();j++) {

						if(reseachLink[i].equals("Login to MyHyundai")) {
							WebElement listMenu = driver.findElementByXPath("(//nav[@class='global-header-dropdown-column global-header-dropdown-more-container']//a)[" +(j+0)+ "]");
							String linksName = listMenu.getText();
							reportStep("Both links " +reseachLink[i]+ " & " +linksName+ " are displayed same", "PASS", true);
						...
						else if(reseachLink[i].equals("Bluelink® Multimedia/Map Updates")) {
							WebElement listMenu = driver.findElementByXPath("(//nav[@class='global-header-dropdown-column global-header-dropdown-more-container']//a)[" +(j+0)+ "]");
							String linksName = listMenu.getText();
							reportStep("Both links " +reseachLink[i]+ " & " +linksName+ " are displayed same", "PASS", true);
							...
							switchToLastWindow();
							sleepTime(10);
							String headerVerify = getTextByXpath(objValue.getProperty("xpath.Bluelink"));
							if(linkNavHeader[k].equals(headerVerify)) {
								reportStep("Link is clicked successfully and page is verified", "PASS", true);
								closeBrowser();
								sleepTime(10);
								switchToParentWindow();
								System.out.println("owner section is completed");
							}
							else {
								reportStep("Link is not displayed successfully and page is not verified", "FAIL", true);
							}
...
//			closeWelcomePopup();
			...
			for(int i = 1;i<=assuranceLinks.size();i++) {
				if(i==1) {
					...
					WebElement linkShopper = driver.findElementByXPath("(//a[@class='global-header-feature-cta'])["+(i+0)+"]");
					 sleepTime(10);
					linkShopper.click();
                System.out.println("linkShopper is clicked");
                     sleepTime(10);
				...
				if(i==2) {
					...
					WebElement linkOwner = driver.findElementByXPath("(//a[@class='global-header-feature-cta'])["+(i+0)+"]");
					 sleepTime(10);
					linkOwner.click();
					System.out.println("linkOwner is clicked");
                     sleepTime(10);
					String shopperPageVerify = getTextByXpath(objValue.getProperty("xpath.shopperPageVerify"));
					verifyTextByXpath(objValue.getProperty("xpath.shopperPageVerify"), shopperPageVerify);
					reportStep("Page is clicked and verified successfully", "PASS", true);
		...
		System.out.println("TC35 completed");
		return this;
...
//		closeWelcomePopup();
		if(driver.findElementByXPath(objValue.getProperty("xpath.shopSectionFooterLink"))!=null) {
			...
			String footerLink[] = {"Shop Hyundai","Find a Dealer","Build & Price","Offers & Promotions","Request a Quote","Search New Inventory",
			...
			String linkNavFooter [] = {"Shop Hyundai","Hyundai Dealerships near","Build Your Hyundai","Offers","Request a quote","Find Your Hyundai",
			...
			for(int i=0;i<=footerLink.length;i++) {
				for(int k=0;k<=linkNavFooter.length;k++) {           
					for(int j = 1;j<=menuList.size();j++) {
						...
						if(footerLink[i].equals("Shop Hyundai")) {
						...
						else if(footerLink[i].equals("Find a Dealer")) {
							...
							listMenu.click();
							System.out.println(linksName+": clicked");
							scrollElementByPixel(objValue.getProperty("xpath.offersPageVerify"));
							String headerVerify = getTextByXpath(objValue.getProperty("xpath.offersPageVerify"));
		...
		System.out.println("TC36 completed");
		return this;
...
//		closeWelcomePopup();
		...
		if(driver.findElementByXPath(objValue.getProperty("xpath.whyHyundaiSectionFooterLink"))!=null) {
			...
			String footerLink[] = {"Overview","America's Best Warranty","Shopper Assurance","Owner Assurance","Hyundai Assurance","Complimentary Maintenance",
					"Happy Drivers","Philanthropy","Made in America","Build for Tomorrow","Social Responsibility","Diversity & Inclusion"};
			String linkNavFooter [] = {"Why Hyundai?","America's Best Warranty","Car shopping made simple.","The benefits of ownership","Hyundai Assurance",
					"Hyundai Complimentary Maintenance","Staying by our drivers’ side","Helping kids fight cancer","Building cars all",
					"Building a better world for tomorrow","Hyundai Corporate Social Responsibility","Hyundai’s Charter Of Inclusion And Diversity"};
			...
			for(int i=0;i<=footerLink.length;i++) {
				for(int k=0;k<=linkNavFooter.length;k++) {           
					for(int j = 1;j<=menuList.size();j++) {
						...
						if(footerLink[i].equals("Overview")) {
						...
						else if(footerLink[i].equals("Diversity & Inclusion")) {
							WebElement listMenu = driver.findElementByXPath("(((//nav[@class='footer-cell'])[3]//li)//a)[" +(j+0)+ "]");
							String linksName = listMenu.getText();
							reportStep("Both links " +footerLink[i]+ " & " +linksName+ " are displayed same", "PASS", true);
							...
							js.executeScript("arguments[0].click();", listMenu);
							switchToLastWindow();
							String headerVerify = getTextByXpath(objValue.getProperty("xpath.footerdiversity"));
							if(linkNavFooter[k].equals(headerVerify)) {
								reportStep("Link is clicked successfully and page is verified", "PASS", true);
								closeBrowser();
								switchToParentWindow();
		...
		System.out.println("TC37 completed");
		return this;
...
//		closeWelcomePopup();
		...
		if(driver.findElementByXPath(objValue.getProperty("xpath.aboutFooterLink"))!=null) {
			...
			String footerLink[] = {"Our Company","COVID-19 Response","Careers","News","Motorsports","Auto Shows","Awards & Accolades","Contact Us"};
			String linkNavFooter [] = {"Our Company","Hyundai’s Response to COVID-19","People come first","What’s Happening","Hyundai Motorsports",
			...
			for(int i=0;i<=footerLink.length;i++) {
				for(int k=0;k<=linkNavFooter.length;k++) {           
					for(int j = 1;j<=menuList.size();j++) {
						...
						if(footerLink[i].equals("Our Company")) {
						...
						else if(footerLink[i].equals("COVID-19 Response")) {
							...
							listMenu.click();
							sleepTime(10);
							String headerVerify = getTextByXpath(objValue.getProperty("xpath.covid19PageVerify"));
							if(linkNavFooter[k].equals(headerVerify)) {
								reportStep("Link is clicked successfully and page is verified", "PASS", true);
							}
		...
		System.out.println("TC38 completed");
		return this;
...
//		closeWelcomePopup();
		if(driver.findElementByXPath(objValue.getProperty("xpath.siteMapSection"))!=null) {
			...
			String footerLink[] = {"Site Map","FAQ's","Accessibility","Terms of Use","Legal","Privacy Policy","Vehicle Technologies and Services Privacy Notice",
					"Do not sell my information","AdChoices"};
			String linkNavFooter [] = {"Site Map","Frequently Asked Questions","Accessibility and inclusion at HyundaiUSA.com","Terms of Use","Hyundai Motor America Legal Disclaimer",
					"Hyundai Motor America Privacy Policy","Vehicle Technologies and Services Privacy Notice","Personal Information Request","Interest Based Advertising Preferences"};
			...
			for(int i=0;i<=footerLink.length;i++) {
				for(int k=0;k<=linkNavFooter.length;k++) {           
					for(int j = 1;j<=menuList.size();j++) {
						...
						if(footerLink[i].equals("Site Map")) {
						...
						else if(footerLink[i].equals("Accessibility")) {
							sleepTime(20);
							WebElement listMenu = driver.findElementByXPath("(//ul[@class='footer-privacy-container']//li)[" +(j+0)+ "]");
							String linksName = listMenu.getText();
							reportStep("Both links " +footerLink[i]+ " & " +linksName+ " are displayed same", "PASS", true);
							...
							listMenu.click();
							sleepTime(20);
							String headerVerify = getTextByXpath(objValue.getProperty("xpath.accpageVerify"));
							if(linkNavFooter[k].equals(headerVerify)) {
								reportStep("Link is clicked successfully and page is verified", "PASS", true);
							}
							else {
						...
						else if(footerLink[i].equals("Vehicle Technologies and Services Privacy Notice")) {
							sleepTime(40);
							WebElement listMenu = driver.findElementByXPath("(//ul[@class='footer-privacy-container']//li)[" +(j+0)+ "]");
							String linksName = listMenu.getText();
							reportStep("Both links " +footerLink[i]+ " & " +linksName+ " are displayed same", "PASS", true);
		...
		System.out.println("TC40 completed");
		return this;
	}
...
//		closeWelcomePopup();
		if(driver.findElementByXPath(objValue.getProperty("xpath.footerLogos"))!=null) {
			...
			for(int i = 1;i<=footerLogosLink.size();i++) {
				if(i==1) {
					...
					System.out.println(motorPage);
					verifyTextByXpath(objValue.getProperty("xpath.motorsPageVerify"), motorPage);
					reportStep("Performance Page is verified successfully", "PASS", true);
				}
		...
		System.out.println("TC39 completed");
		return this;
...
//		closeWelcomePopup();
		...
		System.out.println("TC41 completed");
		return this;
	}
