...
public class VehicleBrowse extends Projectcommonmethodes{
	...
	public VehicleBrowse(RemoteWebDriver driver) {
...
//		sleepTime(5);
		...
		clickByXpathjs(objValue.getProperty("xpath.VehicleNav"), "Vehicles navigation");
		sleepTime(5);
		...
		List<WebElement> categories = driver.findElements(By.xpath("//h2[@class='vbws-group-title']"));
		int countCat = categories.size();
        System.out.println(countCat);
		for(int i=1;i<=countCat;i++) {

			WebElement cateogry1 = driver.findElement(By.xpath("(//h2[@class='vbws-group-title'])["+i+"]"));
	...
	public VehicleBrowse allCategoryVehicleList(String Testcasenumber) throws IOException {
		...
		for ( int i=1;i<=vehiclelist.size();i++)
		{
			...
			System.out.println("Vehicle Compare :"+vehcompare);
		...
		System.out.println("Browse TC completed");
		return this;
...
//		sleepTime(5);
		...
		List<WebElement> suvsVehicleList = driver.findElements(By.xpath("//div[@class='vbws-group vbws-group-active']/div"));
		...
		for(int i=1;i<=15;i++) {
			...
			if(vehcompare.equalsIgnoreCase(SuvvehicleListArray[i])) {
				...
				reportStep(SuvvehicleListArray[i]+" vehicle category displayed in the Vehicle Browse page", "PASS", true);
		...
		System.out.println("Category TC completed");
		return this;
	}
...
//	closeWelcomePopup();
	...
	clickByXpathjs(objValue.getProperty("xpath.sedansVehCategory"), "Sedans category Vehicle");
		...
		String sedansVehicleList = ReadExcelData.getdata(Testcasenumber, "SedansVehicleList");
		System.out.println("sedansVehicleList :"+sedansVehicleList);

		String[] sedansvehicleListArray = sedansVehicleList.split(",");
		
		List<WebElement> sedansVehList = driver.findElements(By.xpath("//div[@class='vbws-group vbws-group-active']/div"));
		WebElement vehCategory = driver.findElement(By.xpath("(//div[@class='vbws-group vbws-group-active']/div)[1]"));
		String categoryName = vehCategory.getText().trim();
		
		int vehSedans = sedansVehList.size();
		int k=20;
for(int j=0;j<vehSedans;j++) {
		for(int i=16;i<k;i++) {
			...
			if(vehcompare.equalsIgnoreCase(sedansvehicleListArray[j])) {
				...
				reportStep(sedansvehicleListArray[j]+" vehicle category displayed properly", "PASS", true);
...
System.out.println("Browse Page TC completed");
		return this;
	}
...
//	sleepTime(5);
	clickByXpathjs(objValue.getProperty("xpath.compactVehCategory"), "Compact category Vehicle");
        ...
        System.out.println(compactsVehicles);
		String[] compactsvehicleListArray = compactsVehicles.split(",");
		
		List<WebElement> compactsVehicleList = driver.findElements(By.xpath("//div[@class='vbws-group vbws-group-active']/div"));
		WebElement vehCategory = driver.findElement(By.xpath("(//div[@class='vbws-group vbws-group-active']/div)[1]"));
		String categoryName = vehCategory.getText().trim();
		System.out.println(categoryName);
		int vehCompacts = compactsVehicleList.size();
		System.out.println("vehCompacts :"+vehCompacts);
		int k=25;
for(int j=0;j<vehCompacts;j++) {
		for(int i=21;i<=k;i++) {
			...
			if(vehcompare.equalsIgnoreCase(compactsvehicleListArray[j])) {
				...
				reportStep(compactsvehicleListArray[j]+" vehicle category displayed properly", "PASS", true);
...
//	closeNotificationPopup();
	...
	List<WebElement> compactVehicleList = driver.findElements(By.xpath("//div[@class='vbws-group vbws-group-active']/div"));
...
//	closeNotificationPopup();
	...
	if(driver.findElement(By.xpath(objValue.getProperty("xpath.compareLink")))!=null) {
		...
		System.out.println("Compare link is clicked from Compare page");
	}
	else {
		reportStep("Compare link is not clicked from Compare page", "FAIL", true);
	}
	System.out.println("Compare TC completed");
	return this;
}
...
public VehicleBrowse compareVehiclesPopup(String Testcasenumber) throws IOException {
	...
	System.out.println("Popup TC completed");
	return this;
}
